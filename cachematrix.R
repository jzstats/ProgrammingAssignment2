...
## a computational proccess under 2 assumptions:
##      * we want to calculate the inverse of some matrices,
##        and this makes our process slow (matrix invertions is costly)
...
##      0. by default the argument 'x' get initialiazed with 'x = matrix()'
##      1. uses force to make the evnironment store the value of matrix 'x'
...
##         are not lost because the object will remember the enviroment in 
...
##         to a varible with name 'inverse_x'
##      2. checks if 'it'inverse_x' is not NULL, so a value is already cached,
...
## is about replacing the workflow:
...
makeCacheMatrix <- function(x = matrix()) {
        ...
        ### gets initiallized here
        inverse_x <- NULL
        
        ### ACCESSORS AND MUTATORS
        #### Initiallize the acsessors (getters)
        ...
        #### Initiallize the mutators (setter)
        ...
        set_matrix <- function(new_x) {
                ...
                # so the opperator '<<-' after failing to find matrix x,
